...
def compute_score(solution_str, ground_truth, exp_name, index, method='strict',
    max_think_reward = 0.0, corresponding_max_think_length = 80, fail_execute_score = 0.01, compile_score=0.2, cheatby_less_ensures_score = 0.0, cheatby_more_assume_score=0.25, score=1., num_examine=0):
    ...
    if not check_no_cheat_by_ensure_true(complete_code):
        if do_print:
    ...
    if ' 0 errors' in return_dict["out"]:
        ...
        ratio = min(ratio, 1)
        ratio = math.exp(ratio) / math.exp(1)
        return compile_score + think_score + 0.2 + 0.6 * ratio
    elif ' 1 errors' in return_dict["out"]:
        ratio = num_ensures / num_gt_ensures
        # use exponential decay
        ratio = min(ratio, 1)
        ratio = math.exp(ratio) / math.exp(1)
        return compile_score + think_score + 0.1 + 0.3 * ratio
    elif ' 2 errors' in return_dict["out"]:
        ratio = num_ensures / num_gt_ensures
        # use exponential decay
        ratio = min(ratio, 1)
        ratio = math.exp(ratio) / math.exp(1)
        return compile_score + think_score + 0.1 * ratio
...
def compute_success_rate(solution_str, ground_truth, exp_name, index, method='strict', fail_execute_score = 0.0, compile_score=0.0, cheatby_less_ensures_score = 0.0, cheatby_more_assume_score=0.0, score=1.):
    """The scoring function for countdown task.
...
def compute_score_with_info(input_tuple):
    """The scoring function for countdown task.
    
    Args:
        solution_str: input_code, in well form
        ground_truth: used to 
        compile_score: the score for correct format
        score: the score for the correct answer
    """
    fail_execute_score = 0.01
    compile_score=0.2
    cheatby_less_ensures_score = 0.0
    cheatby_more_assume_score=0.1
    score=1.0
    code, complete_code = input_tuple
    ...
    if code == "" or code is None:
        # if do_print:
        #     print(f"No codestring found")
        return 0, "Execution result: No codestring found.\n"
    
    # print(code)
    
    return_dict = execute("dafny verify", "dfy", code, timeout_sec=30)
    # print(return_dict)
    info = return_dict["out"]


    if 'parse errors detected' in return_dict["out"]:
        # if do_print:
            # print("Execution result: Parse Errors Detected.")
        return fail_execute_score, "Execution result: Parse Errors Detected.\n" + info
    ...
    if not check_no_cheat_by_less_ensure(complete_code, tidied_code)[0]:
        ...
        return cheatby_less_ensures_score, "Execution result: Parse Errors Not Detected. Cheat by less ensure.\n" + info
    ...
    if '0 errors' in return_dict["out"]:
        ...
        if not check_no_cheat_by_more_assume(complete_code, tidied_code)[0]:
            ...
            return cheatby_more_assume_score, "Execution result: Parse Errors Not Detected. Cheat by assume.\n"+ info
        else:
            # if do_print:
                # print("Execution result: Parse Errors Not Detected. Successfully verified.")
            
            return score, "Execution result: Parse Errors Not Detected. Successfully verified.\n" + info
    else:

        if (return_dict["status"] != 35072) and (return_dict["status"] != 31744):
            # print("Execution result: Parse Errors Not Detected. Failed to be verified.")
            info = "Execution result: Parse Errors Not Detected. Failed to be verified.\n" + info
        else:
            # print("Execution result: Parse Errors Not Detected (likely not detected since out of time limit). Failed due to time limit.")
            info = "Execution result: Parse Errors Not Detected (likely not detected since out of time limit). Failed due to time limit.\n" + info
        # error in execution.
        return compile_score, info
