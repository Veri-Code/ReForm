...
class FSDPSFTTrainer(object):
    ...
    def training_step(self, batch: TensorDict):
        ...
        # Reduce grad_norm across dp ranks for accurate logging
        grad_norm_tensor = torch.tensor(grad_norm).cuda()
        torch.distributed.all_reduce(grad_norm_tensor, op=torch.distributed.ReduceOp.AVG)
        
        return {
            'train/loss': step_loss.detach().item(), 
            'train/lr(1e-3)': lr * 1e3,
            'train/grad_norm': grad_norm_tensor.detach().item()
        }
