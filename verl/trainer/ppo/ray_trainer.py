...
class RayPPOTrainer(object):
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            extra_rewards = result["details"]

            scores = reward_tensor.sum(-1).cpu().tolist()
            sample_scores.extend(scores)
            sample_extra_scores.extend(extra_rewards)
        ...
        metric_dict = {}
        extra_metric_dict_list = []

        total_len = len(sample_extra_scores)
        for jdx in range(len(sample_extra_scores[0])):
            mean_value = 0
            for kdx in range(total_len):
                mean_value += sample_extra_scores[kdx][jdx]
            metric_dict[f"val/reward_term_{jdx}"] = mean_value / total_len
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        # print("Entering Reward function ", time.time())
                        try:
                        ...
                        # print("Leaving Reward function ", time.time())
